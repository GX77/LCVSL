...
class E2ECompressedGEBDModelMulti(Module):
    def __init__(self, cfg):
        ...
        self.temporal_module1 = GroupSimilarity(cfg, dim=dim,
                                                window_size=self.kernel_size + 2,
                                                group=4,
                                                similarity_func='cosine')

        self.temporal_module2 = GroupSimilarity(cfg, dim=dim,
                                                window_size=self.kernel_size - 2,
                                                group=4,
                                                similarity_func='cosine')
    ...
    def forward(self, inputs, targets=None):
        ...
        feats1 = SPoS(feats, self.temporal_module1, self.kernel_size + 2)  # b c t
        feats2 = SPoS(feats, self.temporal_module2, self.kernel_size - 2)  # b c t
        feats = sum([feats, feats1, feats2]) / 3
