...
class E2ECompressedGEBDModelMeixue(Module):
    def __init__(self, cfg):
        ...
        # self.register_buffer('k3', torch.tensor([1.0 / 3.0] * 3).reshape(1, 1, -1))
        # self.register_buffer('k5', torch.tensor([1.0 / 5.0] * 5).reshape(1, 1, -1))
        # self.register_buffer('k7', torch.tensor([1.0 / 7.0] * 7).reshape(1, 1, -1))
        self.register_buffer('k10', torch.tensor([1.0 / 10.0] * 10).reshape(1, 1, -1))
    ...
    def forward(self, inputs, targets=None):
        ...
        if self.training:
            ...
            pred = logits[:, 0, :].contiguous().view(-1)[frame_mask][None, None, :]
            gt = targets[:, :, 0].view(-1)[frame_mask][None, None, :]

            # k3_loss = F.l1_loss(F.conv1d(pred, self.k3), F.conv1d(gt, self.k3))
            # k5_loss = F.l1_loss(F.conv1d(pred, self.k5), F.conv1d(gt, self.k5))
            # k7_loss = F.l1_loss(F.conv1d(pred, self.k7), F.conv1d(gt, self.k7))
            k10_loss = F.l1_loss(F.conv1d(pred, self.k10), F.conv1d(gt, self.k10))

            # l_target = targets[:, :, 1].view(-1)
            # l_target_mask = l_target != -1
            # l_loss = F.l1_loss(logits[:, 1, :].contiguous().view(-1)[l_target_mask], l_target[l_target_mask])
            #
            # r_target = targets[:, :, 2].view(-1)
            # r_target_mask = r_target != -1
            # r_loss = F.l1_loss(logits[:, 2, :].contiguous().view(-1)[r_target_mask], r_target[r_target_mask])

            # loss_dict = {'loss': loss, 'l_loss': l_loss * 0.1, 'r_loss': r_loss * 0.1}
            # loss_dict = {'k3_loss': k3_loss, 'k5_loss': k5_loss, 'k7_loss': k7_loss}
            loss_dict = {'k10_loss': k10_loss}

            return loss_dict
        scores = logits[:, 0, :].flatten(1)
