...
class UpsampleUpdatingModel2(Module):
    ...
    def forward(self, imgs, i_features_list, p_motions):
        ...
        p_features_list = self.mv_backbone.extract_features(einops.rearrange(p_motions, 'bn gop c h w -> (bn gop) c h w'))

        # torch.Size([100, 256, 56, 56]) torch.Size([300, 256, 56, 56])
        # torch.Size([100, 256, 28, 28]) torch.Size([300, 256, 28, 28])
        # torch.Size([100, 256, 14, 14]) torch.Size([300, 256, 14, 14])
        # torch.Size([100, 256, 7, 7]) torch.Size([300, 256, 7, 7])

        p_vectors_list = []
        for i, (i_features, p_features) in enumerate(zip(i_features_list, p_features_list)):
            p_offsets = self.offset_predictor(p_features)
            ...
            p_motions_flatten = einops.rearrange(p_offsets, 'b n p_gop c h w -> (b n p_gop) c h w')

            p_features = self.de_conv1(i_features_flatten, p_motions_flatten)
            p_features = self.motion_convs(p_features)

            p_features = einops.rearrange(p_features, '(b n k) c h w -> b n k c h w', b=B, n=num_gop)
            p_features = F.adaptive_avg_pool2d(p_features, 1).flatten(3)  # b n k c

            p_vectors_list.append(p_features)

            # if is_main_process():
            #     print(i_features.shape, p_features.shape, p_features.shape)

        # quit()

        p_features = sum(p_vectors_list)

        return p_features, None
...
class E2ECompressedGEBDModel(Module):
    ...
    def forward(self, inputs, targets=None):
        ...
        i_features_list = self.extract_features(einops.rearrange(i_imgs, 'b t c h w -> (b t) c h w'))  # (32, 2048, 7, 7)
        ...
        if self._use_residual:
            res_feats = self.res_backbone.extract_features(einops.rearrange(p_res, 'bn gop c h w -> (bn gop) c h w'))[0]
        ...
        if self._use_mv_as_deconv_params:
            p_features, p_features_2d = self.deform_updating_modules(imgs, i_features_list, p_motions)
        else:
            p_features = self.mv_backbone.extract_features(einops.rearrange(p_motions, 'bn gop c h w -> (bn gop) c h w'))[0]
        ...
        # i_features = self.trans_imgs_embedding(i_features)
        i_features = einops.rearrange(i_features_list[-1], '(b n) c h w -> b n c h w', b=B)  # (4, 8, 512, 7, 7)
